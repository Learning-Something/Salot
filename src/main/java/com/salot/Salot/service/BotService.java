    ...
    public void execute() {
        ...
        for (Update mensagens : ListaDeMensagens) {
            ...
            if (mensagens.message().text().toUpperCase().equals("/START")) {
                disparador.dispararMensagemString(
                        "Bot iniciado\n"
                                + "Olá  " + mensagens.message().chat().firstName()+ ", me chamo Salot, sou um bot de um jovem estudante da Universidade"
                                ...
                                + "-Contar uma piada\n\n" + "Caso queira ouvir uma piada, digite ''piada''",
                        mensagens);

            }
            if (mensagens.message().text().toUpperCase().equals("PIADA")) {
                disparador.dispararMensagemString(
                        "Pensei em contar uma de química para voce " + "mas acho que não terá reação", mensagens);
            }
